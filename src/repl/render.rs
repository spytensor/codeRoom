...
/// (role badge on the left, status on the right) but stretches
...
fn handoff_banner(role: &str, host_role: &str, status: &str, width: usize) -> String {
    ...
    // 2 cells for the left inset, role-label width, 1 separator space at each
...
/// parent is the role being quoted. The quote is intentionally one
/// line: the full parent message is already visible above and repeated
/// multi-line quote blocks quickly turn the transcript into a wall.
///
/// ```text
///   @security ↲ @host · "look at src/server, focus on the routing layer ..."
/// ```
///
/// `snippet` is one-line collapsed and truncated so a long parent reply
...
pub(super) fn format_reply_quote(
    ...
    let prefix = format!("  {child_label} {arrow} {parent_label} · ");
    let plain_prefix = format!("  @{child_role} <- @{parent_role} · ");
    let snippet_budget = width
        .saturating_sub(UnicodeWidthStr::width(plain_prefix.as_str()))
        .max(20);
    let snippet_oneline = one_line(parent_text);
    let snippet = truncate_inline(&snippet_oneline, snippet_budget);
    let quote = format!("\"{snippet}\"").with(output::DIM).to_string();
    format!("{prefix}{quote}")
}
